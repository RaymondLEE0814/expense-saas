...
| 단계 | 내용 | 사용자 |
|------|------|--------|
| Phase 1 | 자사((주)슈퍼런 그룹) 내부 사용 | 관리자 + 3개사 담당자 |
| Phase 2 | 클로즈드 베타 (지인 스타트업) | 베타 조직 |
| Phase 3 | 퍼블릭 SaaS 출시 | 일반 스타트업 |

> **개발 방침**: 처음부터 **멀티 테넌트 아키텍처**로 설계한다. Phase 1에서는 (주)슈퍼런 조직 1개만 운영하되, 데이터 모델·인증·권한 체계는 Phase 2/3 확장을 전제로 구축한다.
...
> 멀티 테넌트 아키텍처를 기반으로, **자사((주)슈퍼런) 사용에 필요한 핵심 기능**을 우선 구현한다.
